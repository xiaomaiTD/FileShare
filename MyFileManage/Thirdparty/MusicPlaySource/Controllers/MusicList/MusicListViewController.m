...
- (void)headerRefreshing {
    self.musicEntities = [MusicEntity arrayOfEntitiesFromArray:[self getAllUploadMusicName]].mutableCopy;
    [self.tableView reloadData];
}

- (NSArray *) getAllUploadMusicName
{
    ...
    [files enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([fileString hasSuffix:@"mp3"]) {
            NSString *fullPath = [NSString stringWithFormat:@"%@/%@",uploadDirPath,fileString];
            NSDictionary *musicDic = @{@"name":fileString.lastPathComponent,
                                       @"cover":@"",
                                       @"artistName":fileString.lastPathComponent,
                                       @"musicUrl":@"",
                                       @"fileName":fileString.lastPathComponent,
                                       @"fullPath":fullPath
                                       };
            [fileModelArray addObject:musicDic];
        }
